...
export KEY_EMAIL=corny@cornelinux.de
export KEY_CN=corny
...
help:
	...
	@echo "			generate a request, ext= user, server, etoken, ocsp"
	@echo
...
initca:
	...
	openssl req -days ${CA_EXPIRE} -new -x509 -keyout ${CA_DIR}/ca.key -out ${CA_DIR}/ca.crt -config ${CONF}
...
req:
	...
	openssl req -config ${CONF} -extensions $(ext) -new -out ${DATA_DIR}/${name}.req -keyout ${DATA_DIR}/${name}.key
	@echo
